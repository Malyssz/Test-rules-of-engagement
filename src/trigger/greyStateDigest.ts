...
const STEAM_URL = "https://store.steampowered.com/feeds/news/app/3978820/";
const YOUTUBE_URL =
  "https://www.youtube.com/feeds/videos.xml?channel_id=UCzxH3ovfNamSg2w046MG_HA";
...
async function fetchText(url: string): Promise<string> {
  const res = await fetch(url, {
    headers: { "User-Agent": "grey-state-news-tracker/1.0" },
  });
...
function buildDigest(dateStr: string, isFirstRun: boolean, steam: FeedResult, youtube: FeedResult): string {
  const lines: string[] = [`# Grey State — Cotygodniowy digest (${dateStr})`, ""];
  ...
  lines.push("", "---", "Źródła: Steam News RSS, YouTube RSS (@Play_ROE).");
...
export const greyStateWeeklyDigest = schedules.task({
  id: "grey-state-weekly-digest",
  ...
  run: async () => {
    ...
    const title = totalNew > 0 ? `Grey State: ${totalNew} nowych newsów!` : "Grey State: brak nowości";
